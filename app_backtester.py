...
if not st.session_state.authenticated:
  ...
  with col_l2:
    ...
    auth_mode = st.radio(
        "Mode",
        ["🔑 Terminal Sign In", "✨ Register Verified Account"],
        horizontal=True,
    ...
    if auth_mode == "🔑 Terminal Sign In":
      with st.form("login_form"):
        ...
        if st.form_submit_button("⚡ UNLOCK QUANTUM TERMINAL"):
          ...
          if username in users and users[username]["pass"] == password:
            ...
            st.error(
                "⛔ Authentication Denied: Invalid Security Key or User ID."
            )
    else:
      with st.form("signup_form"):
        ...
        new_phone = st.text_input(
            "10-Digit Mobile / WhatsApp Number *", placeholder="e.g. 9876543210"
        )
        new_user = st.text_input(
            "Create Operator User ID *", placeholder="e.g. samir_quant"
        )
        new_pass = st.text_input(
            "Create Access Password (Min 4 chars) *", type="password"
        )
...
def calc_indicators(df, params):
  ...
  for i in range(1, len(d)):
    if basic_ub.iloc[i] < final_ub.iloc[i - 1] or c.iloc[i - 1] > final_ub.iloc[
        i - 1
    ]:
    ...
    if basic_lb.iloc[i] > final_lb.iloc[i - 1] or c.iloc[i - 1] < final_lb.iloc[
        i - 1
    ]:
...
is_admin = (
    curr_tier == "Master Admin" or st.session_state.user_info.get("id") == "admin"
)
...
with st.sidebar:
  ...
  symbol = st.selectbox(
      ...
      format_func=lambda x: asset_dict[x],
  )
  ...
  lookback_days = st.slider(
      "Lookback Memory (Days)", 1, max_days, default_days
  )
  ...
  capital = st.number_input(
      "Capital Pool (₹)", value=100000.0, step=10000.0
  )
  ...
  with col_k1:
    target_val = st.number_input(
        "Target (" + ("Pts" if is_idx else "%") + ")",
        value=50.0 if is_idx else 2.5,
        step=5.0 if is_idx else 0.5,
    )
  with col_k2:
    sl_val = st.number_input(
        "Hard SL (" + ("Pts" if is_idx else "%") + ")",
        value=20.0 if is_idx else 1.0,
        step=5.0 if is_idx else 0.2,
    )
...
if not is_admin and curr_tier == "Free Member":
  with st.expander(
      "⚡ UPGRADE TO VIP ALGO TRADER (Click to Expand / Dismiss)",
      expanded=False,
  ):
    st.markdown(
        """
...
with col_run1:
  st.write(
      f"💼 **Active Target:** `{asset_dict[symbol]}` | Strategy:"
      f" **{strategy_type.split('.')[1].strip()}** | Risk Profile: **Risk"
      f" {sl_val}{' Pts' if is_idx else '%'} to Gain"
      f" {target_val}{' Pts' if is_idx else '%'}**"
  )
with col_run2:
...
if is_admin:
  (
      ...
      "📜 Trade Logs",
      "📥 Download Reports",
      "⚡ 1-Click Live Opportunity Radar & Tracker",
...
if execute_btn or "backtest_executed" in st.session_state:
  ...
  with st.spinner("⏳ Loading institutional price matrix..."):
    try:
      df_raw = yf.download(
          symbol, period=f"{lookback_days}d", interval=timeframe, progress=False
      )
      if df_raw.empty or len(df_raw) < 10:
        st.warning(
            "⚠️ No market data returned. Please select a higher timeframe or"
      ...
      ist_time = (
          ...
          else df.index + pd.Timedelta(hours=5, minutes=30)
      )
      ...
      for i in range(2, len(df)):
        ...
        if position is not None:
          move = (
              (curr_spot - position["entry"])
              if position["type"] == "BUY/CE"
              else (position["entry"] - curr_spot)
          )
          opt_move = move if is_idx else ((move / position["entry"]) * 100)

          if opt_move >= target_val:
            pnl = (
                (target_val * qty * delta)
                if is_idx
                else ((target_val / 100) * capital)
            )
            trades.append({
      ...
      with tab_chart:
        ...
        fig.add_hline(
            y=70, line_dash="dash", line_color="rgba(239, 68, 68, 0.4)", row=2, col=1
        )
        fig.add_hline(
            y=30,
            line_dash="dash",
        ...
        config_touch = {
            ...
            "toImageButtonOptions": {
                "format": "png",
                "filename": f"sam_quantum_{symbol}",
                "height": 1080,
                "width": 1920,
                "scale": 2,
            },
      ...
      with tab_metrics:
        if trades:
          ...
          k1.metric(
              "Net Realized PnL",
              f"{'+₹' if net_pnl >= 0 else '-₹'}{abs(net_pnl):,.2f}",
          )
          k2.metric(
              "Win Probability",
              f"{win_rate:.1f}%",
              f"{win_count}W / {len(tdf)-win_count}L",
          )
          ...
          st.plotly_chart(
              fig_equity, use_container_width=True, config=config_touch
          )
      ...
      with tab_trades:
        if trades:
          ...
          st.dataframe(
              pd.DataFrame(trades), use_container_width=True, height=450
          )
        ...
        with col_r1:
          if trades:
            ...
            st.download_button(
                ...
                file_name=f"sam_quantum_{symbol}.csv",
                mime="text/csv",
            )
        with col_r2:
          if trades:
            html_report = (
                '<!DOCTYPE html><html><body style="background:#050811;color:#f1f5f9;font-family:sans-serif;padding:20px;"><h2'
                ' style="color:#38bdf8;">SAM QUANTUM AI - AUDIT</h2><p>Asset:'
                f" <b>{asset_dict[symbol]}</b> | Net PnL:"
                f" <b>₹{net_pnl:,.2f}</b> | Win Rate:"
                f" <b>{win_rate:.1f}%</b></p>{pd.DataFrame(trades).to_html(index=False)}</body></html>"
            )
            st.download_button(
                "📥 DOWNLOAD HTML AUDIT",
                data=html_report,
                file_name=f"sam_quantum_{symbol}.html",
                mime="text/html",
            )
        with col_r3:
          st.download_button(
              "📄 DOWNLOAD MASTER MANUAL (PDF)",
              data=manual_html_doc,
              file_name="sam_quantum_master_manual.html",
              mime="text/html",
          )
    except Exception as e:
  ...
  with tab_reports:
    ...
    st.download_button(
        "📄 DOWNLOAD MASTER OPERATING MANUAL (PDF / PRINT GUIDE)",
        data=manual_html_doc,
        file_name="sam_quantum_master_manual.html",
        mime="text/html",
    )
...
# ==============================================================================
# 👑 ADMIN ONLY: TAB 5 - LIVE RADAR & DYNAMIC ACCOUNTABILITY TRACKER
# ==============================================================================
if is_admin:
  with tab_single_radar:
    st.markdown("### ⚡ Live Opportunity Radar & Trade Accountability Engine")
    st.caption(
        "Scans real-time live markets with multi-confluence checks, dispatches"
        " short alerts, and updates trailing SL / Target completion."
    )

    col_sr1, col_sr2 = st.columns(2)
    with col_sr1:
      radar_asset = st.selectbox(
          "Target Market Feed",
          options=list(asset_dict.keys()),
          format_func=lambda x: asset_dict[x],
          key="single_rad_asset",
      )
      ...
      rd_target = st.number_input(
          "Target (" + ("Pts" if is_rd_idx else "%") + ")",
          value=50.0 if is_rd_idx else 2.5,
          step=5.0 if is_rd_idx else 0.5,
          key="rd_tp",
      )

    with col_sr2:
      radar_tf = st.selectbox(
          "Scanning Resolution", ["1m", "5m", "15m"], index=1, key="rd_tf"
      )
      rd_sl = st.number_input(
          "Hard SL (" + ("Pts" if is_rd_idx else "%") + ")",
          value=20.0 if is_rd_idx else 1.0,
          step=5.0 if is_rd_idx else 0.2,
          key="rd_sl",
      )
      min_conf_single = st.slider(
          "Minimum AI Edge Confidence %", 70, 95, 80, key="rd_conf"
      )

    st.markdown("#### 🌐 Active Open Trades (Accountability Log)")
    if st.session_state.active_radar_trades:
      act_df = pd.DataFrame(
          list(st.session_state.active_radar_trades.values())
      )
      st.table(
          act_df[
              ["asset_name", "action", "entry", "target", "sl", "status", "time"]
          ]
      )
    else:
      st.info(
          "No active trades currently in tracking. Click below to audit live"
          " feed."
      )

    col_btn1, col_btn2 = st.columns(2)
    with col_btn1:
      scan_now = st.button("🚀 AUDIT & BROADCAST NEW SIGNAL")
    with col_btn2:
      update_live_track = st.button(
          "📈 UPDATE RUNNING TRADES (TRAIL SL / TARGET)"
      )

    if scan_now:
      if not is_open:
        st.error(
            f"⛔ Cannot broadcast: {gate_info}. AI strictly prevents sending"
            " alerts during closed hours."
        )
      else:
        with st.spinner(
            f"🔍 Auditing live multi-confluences for {asset_dict[radar_asset]}..."
        ):
          try:
            df_live = yf.download(
                radar_asset, period="2d", interval=radar_tf, progress=False
            )
            if df_live.empty or len(df_live) < 15:
              st.error(
                  "❌ Live feed temporary timeout. Please retry in 5 seconds."
              )
            else:
              ...
              if ema20_v > ema50_v and spot > ema20_v and rsi_v > 54:
                sig = "BUY / CALL (CE) 🟢"
                conf = 88
                logic = f"20/50 EMA Bullish Trend + RSI Momentum ({rsi_v:.1f})"
              elif ema20_v < ema50_v and spot < ema20_v and rsi_v < 46:
                sig = "SELL / PUT (PE) 🔴"
                conf = 88
                logic = f"20/50 EMA Bearish Trend + RSI Drop ({rsi_v:.1f})"
              elif st_p == -1 and st_n == 1 and rsi_v > 50:
                sig = "BUY / CALL (CE) 🟢"
                conf = 92
                logic = "SuperTrend Bullish Reversal Confluence"
              elif st_p == 1 and st_n == -1 and rsi_v < 50:
                sig = "SELL / PUT (PE) 🔴"
                conf = 92
                logic = "SuperTrend Bearish Reversal Confluence"
              ...
              if is_rd_idx:
                ...
                tp = (
                    spot * (1 + (rd_target / 100.0))
                    if "BUY" in sig
                    else spot * (1 - (rd_target / 100.0))
                )
                sl = (
                    spot * (1 - (rd_sl / 100.0))
                    if "BUY" in sig
                    else spot * (1 + (rd_sl / 100.0))
                )
                risk_desc = f"+{rd_target}% | SL: -{rd_sl}%"

              now_ist = datetime.now(pytz.timezone("Asia/Kolkata")).strftime(
                  "%I:%M %p IST"
              )

              if sig == "NEUTRAL" or conf < min_conf_single:
                st.info(
                    "Market is currently consolidative with no high-probability"
                    f" setups meeting {min_conf_single}% confidence threshold."
                )
              else:
                tg_text = (
                    "⚡ <b>SAM QUANTUM AI - LIVE SIGNAL</b> ⚡\n"
                    "━━━━━━━━━━━━━━━━━━━━━\n"
                    f"📊 <b>Asset:</b> {asset_dict[radar_asset]}\n"
                    f"🎯 <b>Action:</b> <code>{sig}</code>\n"
                    f"💵 <b>Entry Spot:</b> {curr_sym}{spot:,.2f}\n"
                    f"🎯 <b>Target:</b> {curr_sym}{tp:,.2f}"
                    f" ({risk_desc.split('|')[0].strip()})\n"
                    f"🛑 <b>Stop Loss:</b> {curr_sym}{sl:,.2f}"
                    f" ({risk_desc.split('|')[1].strip()})\n"
                    f"⏱ <b>Time:</b> {now_ist} | 🧠 <b>Edge:</b> {conf}%\n"
                    "━━━━━━━━━━━━━━━━━━━━━\n"
                    "⚠️ <i>Strict 1:2 R:R. Trail SL on momentum.</i>"
                )

                ok, resp = send_telegram_alert(tg_text)
                if ok:
                  st.session_state.active_radar_trades[radar_asset] = {
                      "asset_name": asset_dict[radar_asset],
                      "action": sig,
                      "entry": spot,
                      "target": tp,
                      "sl": sl,
                      ...
                      "sym": curr_sym,
                      "is_idx": is_rd_idx,
                      "target_dist": rd_target,
                  }
                  st.success(
                      "✅ Live Setup Dispatched to @sam_quantum_signals!"
                      f" (Signal: {sig} @ {curr_sym}{spot:,.2f})"
                  )
                  time.sleep(0.5)
                  st.rerun()
                else:
    ...
    if update_live_track:
      with st.spinner("Auditing open active trades against live spot..."):
        completed = []
        now_ist = datetime.now(pytz.timezone("Asia/Kolkata")).strftime(
            "%I:%M %p IST"
        )

        for r_sym, r_trade in list(
            st.session_state.active_radar_trades.items()
        ):
          try:
            df_check = yf.download(
                r_sym, period="1d", interval="1m", progress=False
            )
            if df_check.empty:
              continue
            ...
            live_spot = float(df_check["Close"].iloc[-1])
            c_sym = r_trade["sym"]
            ...
            target_hit = (
                (live_spot >= tp_p) if "BUY" in action else (live_spot <= tp_p)
            )
            sl_hit = (
                (live_spot <= sl_p) if "BUY" in action else (live_spot >= sl_p)
            )

            halfway = (
                entry_p + ((tp_p - entry_p) * 0.5)
                if "BUY" in action
                else entry_p - ((entry_p - tp_p) * 0.5)
            )
            can_trail = (
                (live_spot >= halfway)
                if "BUY" in action
                else (live_spot <= halfway)
            )

            if target_hit:
              tg_done = (
                  "🎯 <b>TARGET COMPLETED - BOOK FULL PROFIT</b> 🎯\n"
                  "━━━━━━━━━━━━━━━━━━━━━\n"
                  f"📊 <b>Asset:</b> {r_trade['asset_name']}\n"
                  "✅ <b>Status:</b> <code>FULL TARGET HIT 🚀</code>\n"
                  f"💵 <b>Entry:</b> {c_sym}{entry_p:,.2f} ➔ <b>Exit:</b>"
                  f" {c_sym}{live_spot:,.2f}\n"
                  f"⏱ <b>Completed At:</b> {now_ist}\n"
                  "━━━━━━━━━━━━━━━━━━━━━\n"
                  "🤖 <i>Accountability Logged via Sam Quantum AI</i>"
              )
              ...
              tg_sl = (
                  ...
                  "━━━━━━━━━━━━━━━━━━━━━\n"
                  "🤖 <i>Risk Managed via Sam Quantum AI</i>"
              )
              ...
              tg_trail = (
                  "⚡ <b>UPDATE: SHIFT SL TO BREAKEVEN</b> ⚡\n"
                  "━━━━━━━━━━━━━━━━━━━━━\n"
                  f"📊 <b>Asset:</b> {r_trade['asset_name']}\n"
                  f"📈 <b>Price Moved:</b> {c_sym}{entry_p:,.2f} ➔"
                  f" {c_sym}{live_spot:,.2f}\n"
                  f"🛡️ <b>New Stop Loss:</b> {c_sym}{entry_p:,.2f} <code>(COST /"
                  " ZERO RISK)</code>\n"
                  f"⏱ <b>Time:</b> {now_ist}\n"
                  "━━━━━━━━━━━━━━━━━━━━━\n"
                  "🤖 <i>Locking gains & eliminating risk.</i>"
              )
              ...
              st.session_state.active_radar_trades[r_sym]["status"] = (
        ...
        st.success("✅ Trade accountability audit complete!")
        time.sleep(0.5)
        st.rerun()

  # --- TAB 6: ACCESS CONSOLE ---
  with tab_admin_access:
    ...
    with col_u1:
      ...
      st.download_button(
          "📥 EXPORT VERIFIED OPERATORS (CSV)",
          data=csv_users.getvalue(),
          file_name="sam_quantum_users.csv",
          mime="text/csv",
      )
    ...
    with col_u2:
      ...
      removable_users = [
          u for u in st.session_state.users_db.keys() if u != "admin"
      ]
      if removable_users:
        target_del = st.selectbox(
            "Select Account to Ban / Revoke", removable_users
        )
        ...
        target_up = st.selectbox(
            "Select Operator for Tier Upgrade", removable_users, key="up_target"
        )
